...
Mosaic = function(conf)
  ...
  self.ask = function(self, person, corner_img, title, center_img, drops_conf, words, sound, right_goto, wrong_goto, time)
    ...
    -- add image/title
    if person then
      self.person = AnimatedItem(person)
    end
    if corner_img then
...
--    print_table(drops_conf)
    ...
    do
      ...
      do
--        print(row, col)
        local d = DropArea(x, y, TwoStateAnimation(Animation("DropArea")))
          d.y = d.y + d.height / 2
...
--      print(d.x, d.y, d.width, d.height)
    ...
    if #words > 0 then
      local tmp = DropArea(0, 0, TwoStateAnimation(Animation("DropArea")))
      tmp.visible = false
    ...
    -- add button
    self.button = Button(screen_width / 2, conf.button_y, TwoStateAnimation(Animation("Start")))
