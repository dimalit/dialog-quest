...
local i1 = TextBoxItem("Буква «а» в закрытом слоге читается как ")
i1.debugDrawColor=0xff0000ff
i1.id = "item1"
p1:addItem(i1):addItem(VoiceTextItem("ding")):addItem(TextBoxItem(". Такого звука в русском языке нет. Это нечто среднее между русскими «а» и «э». Чтобы произнести этот звук правильно, широко раскрой рот так, будто хочешь сказать «а», и скажи «э». Звук ")):addItem(PhonemicItem("x"))
local im = ImageItem("interface/flask.rttex")
im.scaleX = 1.0
p1:addItem(im)
p1:addItem(TextBoxItem(" очень короткий, его ни в коем случае нельзя тянуть."))
...
local mo = PackAsDragDrop(ImageItem("interface/flask.rttex"))
